  ...
  const validateQRCode = async (data: string) => {
    ...
    try {
      ...
      if (responseBody.valid) {
        const details = [
          responseBody.subscriptionId && `Subscription: ${responseBody.subscriptionId}`,
          responseBody.userId && `User: ${responseBody.userId}`,
          responseBody.expiresAt && `Expires: ${new Date(responseBody.expiresAt).toLocaleDateString()}`
        ].filter(Boolean).join(' | ')
